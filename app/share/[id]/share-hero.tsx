...
export function ShareHero({ collection, agent }: ShareHeroProps) {
  ...
  return (
    <div className="relative h-[25vh] md:h-[30vh] w-full overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-8 relative z-10 h-full flex flex-col justify-end pb-10">
        <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-serif font-medium text-white mb-4 animate-fade-in-up text-shadow-md">{collection.name}</h1>
        ...
        {collection.description ? (
          <p className="text-base sm:text-lg md:text-xl text-white/90 max-w-3xl animate-fade-in-up text-shadow-sm" style={{animationDelay: '200ms'}}>
            {collection.description}
          </p>
        ) : (
          <p className="text-base sm:text-lg md:text-xl text-white/90 max-w-2xl animate-fade-in-up text-shadow-sm" style={{animationDelay: '200ms'}}>
