    ...
    async (file: File) => {
      try {
        ...
        toast({
          variant: "destructive",
          title: "Ошибка загрузки",
          description: "Произошла непредвиденная ошибка при загрузке изображения вида из окна",
        })
        return null
      }
    },
  ...
  const removeFile = useCallback(async () => {
    if (file?.url) {
      try {
        ...
        console.error("Ошибка при удалении вида из окна из хранилища:", err)
      }
    }
      ...
      {!file ? (
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center space-y-2">
            ...
            <div className="text-sm font-medium text-luxury-black dark:text-white theme-transition">Перетащите изображение вида из окна сюда или нажмите для выбора</div>
            <div className="text-xs text-gray-500 dark:text-gray-300 theme-transition">Загрузите изображение вида из окна (макс. 10МБ)</div>
          </div>
        </div>
      ) : (
